...
public class RecordFlowTests
{
    ...
    private static (FiniteStateMachine<BotContext> fsm, BotContext ctx, SpyMessenger spy) StartRecord()
    {
        ...
        fsm.Process(new Event(BotEvents.NewMessage, new ChatMessage(Recorder, "record", tagsBot: true)), ctx);
        spy.Log.Clear();
        return (fsm, ctx, spy);
    }
    ...
    public void Recording_accumulates_speak_without_transition_or_output()
    {
        ...
        fsm.Process(GoBroadcasting(), ctx); // Waiting → Recording
        spy.Log.Clear();

        var result = fsm.Process(Speak("大家早安"), ctx);

        Assert.Equal(TriggerResult.NotConsumed, result); // 累積不轉移 → 冒泡靜默
        Assert.Equal("Record", fsm.CurrentState.Id);
    ...
    public void Multiple_speaks_accumulate_in_order()
    {
        ...
        fsm.Process(GoBroadcasting(), ctx);

        fsm.Process(Speak("一"), ctx);
        fsm.Process(Speak("二"), ctx);
        fsm.Process(Speak("三"), ctx);
    ...
    public void Stop_broadcasting_emits_replay_clears_buffer_and_loops_to_waiting()
    {
        ...
        fsm.Process(StopBroadcasting(), ctx);
    ...
    public void Loop_back_to_waiting_then_record_again_starts_from_empty_buffer()
    {
        ...
        fsm.Process(GoBroadcasting(), ctx);
        fsm.Process(Speak("一"), ctx);
        fsm.Process(StopBroadcasting(), ctx); // → Waiting,buffer 清空
        spy.Log.Clear();

        fsm.Process(GoBroadcasting(), ctx);   // 再進 Recording
    ...
    {
        var (fsm, ctx, _) = StartRecord();
        fsm.Process(GoBroadcasting(), ctx); // 在 Recording

        var result = fsm.Process(StopRecordingCmd(), ctx); // 限錄音者的指令
    ...
    public void Stop_recording_while_recording_emits_replay_then_normal()
    {
        ...
        fsm.Process(StopRecordingCmd(), ctx);
    ...
    public void Speak_in_other_state_is_silent_and_not_accumulated()
    {
        ...
        var ctx = new BotContext(spy, initialTokenQuota: 100);
        var fsm = WaterballBot.Define();
        fsm.CurrentState.OnEntry(ctx); // Normal，沒進 Record

        var result = fsm.Process(Speak("路過"), ctx);
